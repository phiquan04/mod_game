    ...
    public static void process(Player p, Message m2) throws IOException {
        ...
        if (value == 0) { // ok
            switch (id) {
                ...
                case 63: {
                    if (p.data_yesno != null && p.data_yesno.length == 1 && p.data_yesno[0] == 63) {
                        if (p.party != null) {
                            if (p.party.list.get(0).equals(p)) {
                                ...
                                for (int i = 0; i < p.party.list.size(); i++) {
                                    ...
                                    if (p0.get_key_boss() < 2) {
                                        Service.send_box_ThongBao_OK(p, p0.name + " không đủ 2 chìa khoá phó bản");
                                        checkKey = false;
                                        break;
                                    }
                                }
                                if (checkKey) {
                                    ...
                                    int index_mob = -2;
                                    // create map
                                    Map mapTemplate = Map.get_map_by_id(9990)[0];
                                    Map map_boss = new Map();
                                    map_boss.template = mapTemplate.template;
                                    map_boss.zone_id = (byte) 0;
                                    map_boss.list_mob = new int[0];
...
//                            p.map_boss_info.mob.add(mob_add);
                ...
                case 10: {
                    ...
                    p.update_ngoc(1000);
                    p.update_money();
                    Service.send_box_ThongBao_OK(p, "Bạn đã đổi thành công 1000 ruby khoá.");
                    break;
                }
                case 9: {
                    if (p.data_yesno != null && p.data_yesno.length == 1) {
                ...
                case 734: {
                    if (p.item.total_item_bag_by_id(4, id) > 0) {
                        Pet.addPetToPlayer(p, (short) (id - 689));
