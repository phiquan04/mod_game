...
public class Clan {
    ...
    public void chat_on_board(short id, String name, String s, int typeChat) throws IOException {
        ...
        public static synchronized void remove_clan(Player p,Clan clan) throws IOException {
       if (p.clan != null && p.clan.members.get(0).name.equals(p.name)) {
        ...
        Clan.ENTRY.remove(clan);
        Clan.update();
        for (Clan_member member : clan.members) {
            Player player = Map.get_player_by_name_allmap(member.name);
            if (player != null) {
                player.clan = null; 
                Service.send_box_ThongBao_OK(player, "Bang hội của bạn đã bị giải tán.");
            }
        }
