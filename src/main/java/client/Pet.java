...
public class Pet {
    ...
    public static void addPetToPlayer(Player p, short petTemplateId) throws IOException {
        ...
        if (petTemplate != null) {
            MyPet existingPet = getExistingPet(p, petTemplateId);
            if (existingPet != null) {
                // Tạo một bản sao của Pet Template
                Pet newPetTemplate = new Pet();
                newPetTemplate.id_pet = petTemplate.id_pet;
                newPetTemplate.icon = petTemplate.icon;
                newPetTemplate.frame = petTemplate.frame;
                newPetTemplate.name = petTemplate.name;
                newPetTemplate.type = petTemplate.type;
                newPetTemplate.option_pet = new ArrayList<>();
                
                //thay cs pet
                addDefaultOptionsToPet(newPetTemplate);

                existingPet.template = newPetTemplate;
            } else {
                existingPet = new MyPet(petTemplateId, petTemplate);
                p.my_pet.add(existingPet);
            }
            limitOptions(p);
            show_inven(p);
    ...
    private static void limitOptions(Player p) {
        for (MyPet pet : p.my_pet) {
            while (pet.template.option_pet.size() > 4) {
                pet.template.option_pet.remove(pet.template.option_pet.size() - 1);
            }
        }
    }

    private static MyPet getExistingPet(Player p, short petTemplateId) {
        for (MyPet pet : p.my_pet) {
            if (pet.template.id_pet == petTemplateId) {
                return pet;
            }
        }
        return null;
    ...
    private static void addDefaultOptionsToPet(Pet pet) {
        ...
        for (int i = 0; i < numOptions; i++) {
            ...
            usedIds.add(id);
            short param = (short) Util.random(1, 5); // Giá trị ngẫu nhiên cho param
           // short param = (short) 2000;
