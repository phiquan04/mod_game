            ...
            case 1: {
                ...
                if (num == 1 && cat == 3 && p.trade_target != null) { // add item vao
                    ...
                    long beri_quant = ((long) num * 12L) / 10L;
                     if (p.getBeri0() < beri_quant) {
                        Service.send_box_ThongBao_OK(p, "Bạn không đủ " + beri_quant + " Beri nạp để giao dịch. Beri nạp hiện tại: " + p.getBeri0()
                                + ". Yêu cầu beri nạp trên " + beri_quant + " mới có thể tiếp tục giao dịch");
                        return;
                    }
                    if (p.get_vang() < beri_quant) {
                        Service.send_box_ThongBao_OK(p, "Bạn không đủ " + beri_quant + " beri để giao dịch. Beri hiện tại: " + p.get_vang()
                                + ". Yêu cầu beri trên " + beri_quant + " mới có thể tiếp tục giao dịch");
                        return;
                    }
                    if (p.get_vang() < beri_quant) {
            ...
            case 4: { // accept
                if (id == -1 && cat == -1 && num == -1 && p.trade_target != null && p.is_lock_trade
                    ...
                    if (p.get_ngoc() < p.fee_trade) {
                        Service.send_box_ThongBao_OK(p, "Bạn không đủ " + p.fee_trade + " Ruby để trả phí giao dịch. Ruby hiện tại: " + p.get_ngoc()
                                + ". Yêu cầu ruby trên " + p.fee_trade + " mới có thể tiếp tục giao dịch");
                        return;
                    }
                    
                    if (p.getBeri0() < p.money_trade) {
                        Service.send_box_ThongBao_OK(p, "Bạn không đủ " + p.money_trade + " Beri nạp để giao dịch. Beri nạp hiện tại: " + p.getBeri0()
                                + ". Yêu cầu beri nạp trên " + p.money_trade + " mới có thể tiếp tục giao dịch");
                        return;
                    }
                    if (p.get_vang() < p.money_trade) {
                        Service.send_box_ThongBao_OK(p, "Bạn không đủ " + p.money_trade + " beri để giao dịch. Beri hiện tại: " + p.get_vang()
                                + ". Yêu cầu beri trên " + p.money_trade + " mới có thể tiếp tục giao dịch");
                        return;
                    }
