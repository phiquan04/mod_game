...
public class UpgradeSuperItem {
    public static int[][] MATERIAL = new int[][] { //
            new int[] {100, 5, 1_000_000, 30}, // 1
            new int[] {100, 10, 1_500_000, 30}, // 2
        ...
        if (type == 4 && beri_gem == 0 && num == 0) { // add item3
            ...
            if (it_select != null) {
                ...
                if (p.data_super_upgrade[1] >= 1 && p.data_super_upgrade[1] <= 5) {
                    percent_decrease_level -= (p.data_super_upgrade[1]) * 17;
                ...
                //
                boolean suc = percent_suc > Util.random(120 + it_select.levelup * 2);//ti le theo cap
                if(it_select.levelup >= 16){
                    suc = percent_suc > Util.random(1000);//tam 5 10 / 1000 -> 1%
                }
                System.out.println("percent_suc =" +percent_suc);
                Message m = new Message(66);
                if (suc) {
                    it_select.levelup++;
    ...
    private static int get_material(int type, Item_wear it_select) {
        ...
        int delta = 0;
        int index = (it_select.template.level / 10) - 1;
    ...
    private static boolean check_it_can_upgrade_super(Player p, Item_wear it_select)
        ...
        if (it_select.template.level < 10 || it_select.template.level > 100) {
            Service.send_box_ThongBao_OK(p,
