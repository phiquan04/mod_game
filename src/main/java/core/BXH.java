...
//                    short[] part = new short[] {-1, -1, -1};
                    if (p0 != null) {
                        ...
                        if (p0.item.it_body[0] != null) {
                             temp.part[2] = p0.item.it_body[0].template.part;
                        }
                        if (p0.item.it_body[5] != null) {
                             temp.part[1] = p0.item.it_body[5].template.part;
                        }
                        if (p0.item.it_body[3] != null) {
                             temp.part[0] = p0.item.it_body[3].template.part;
                        }
                    }
    ...
    private static void updateWanted() {
        ...
        try {
            ...
            ps = connection.prepareStatement(
                    "SELECT `id`, `name`, `clazz`, `wanted_point`, `body`, `it_body`, `fashion`, `site` FROM `players` ORDER BY `wanted_point` DESC LIMIT 50;");
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                temp.part = new short[]{-1, -1, -1};
                if (it != null) {
               if (it[0] != null) {
                  temp.part[2] = it[0].template.part;
               }
               if (it[5] != null) {
                  temp.part[1] = it[5].template.part;
               }
               if (it[3] != null) {
                  temp.part[0] = it[3].template.part;
               }
            }
                list_add.add(temp);
            }
    ...
    private static void updatePVP() {
        ...
        try {
            connection = SQL.gI().getCon();
            ps = connection.prepareStatement(
                    "SELECT `id`, `name`, `clazz`, `pvppoint`, `body`, `it_body`, `fashion`, `site` FROM `players` ORDER BY `pvppoint` DESC LIMIT 50;");
            rs = ps.executeQuery();
            while (rs.next()) {
                InfoMemList temp = new InfoMemList();
                temp.id = rs.getInt("id");
